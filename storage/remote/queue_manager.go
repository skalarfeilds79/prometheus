...
type queue struct {
	...
	// Since we know there is a limited number of batches out, using a stack is
	// easy and persistent across GC compared to a sync.Pool
	batchPool [][]interface{}
	poolMux   sync.Mutex
}
...
func newQueue(batchSize, capacity int) *queue {
	...
	return &queue{
		...
		batchPool:  make([][]interface{}, 0, batches),
	}
}
...
// Return adds the batch buffer back to the internal pool.
func (q *queue) Return(batch []interface{}) {
	q.poolMux.Lock()
...
// Shutdown stops the queue and flushes any samples. No appends can be made
// after this is called.
func (q *queue) Shutdown(done <-chan struct{}) {
	if len(q.batch) > 0 {
		select {
		case q.batchQueue <- q.batch:
		case <-done:
